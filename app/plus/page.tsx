      ...
      <AppShell>
        <div className="flex-1 overflow-y-auto px-4 md:px-10 py-8">
          <header className="text-center max-w-2xl mx-auto mb-10">
            <h1 className="font-display text-2xl md:text-3xl mb-2">Get unlimited access on Atelier</h1>
            <p className="text-sm text-parchment/50">
              Higher tiers unlock more premium engines and memory — billing is not active yet.
            </p>
            <p className="text-xs text-gold/80 mt-3">{EARLY_ACCESS_MESSAGE}</p>
            {subscribeError && <p className="text-xs text-rose-400 mt-3">{subscribeError}</p>}
          </header>
          ...
          <div className="flex justify-center gap-2 mb-10">
            {(
              ...
              <button
                ...
                type="button"
                onClick={() => setCycle(id)}
                className={`px-5 py-2 rounded-full text-sm focus-ring border ${
                  cycle === id ? "bg-white/10 border-white/25" : "border-white/10 text-parchment/50"
                }`}
              >
                {label}
              </button>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 max-w-6xl mx-auto">
            {MEMBERSHIP_TIERS.map((tier) => {
              ...
              return (
                <div
                  key={tier.id}
                  className={`rounded-2xl bg-surface-card p-5 flex flex-col border ${cardClass} ${
                    tier.highlight ? "ring-1 ring-gold/40" : ""
                  }`}
                >
                  <div className="mb-4">
