...
export default function ExplorePage() {
  ...
  // Remix makes the current user a full, independent, freely-editable copy
  // of a public character (backend: POST /:id/remix) — for when someone
  // wants more than the chat-scoped personalization (name/portrait/backdrop
  // overrides) covers and actually wants to own and permanently rework the
  // character. The original stays exactly as-is for its creator and
  // everyone else.
  async function onRemix(target: ExploreCardCharacter) {
    setRemixError("");
    setRemixingId(target.id);
    try {
      const res = await apiFetch(`/api/characters/${target.id}/remix`, { method: "POST" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setRemixError(data.error || "Couldn't remix this character.");
        return;
      }
      router.push(`/characters/${data.character.id}/edit`);
    } catch {
      setRemixError("Couldn't reach the server.");
    } finally {
      setRemixingId(null);
    }
  }
      ...
      <AppShell>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-4 md:px-8 pt-6 pb-4">
            ...
            {remixError && (
              <p className="mt-4 text-sm text-rose bg-rose/10 border border-rose/30 rounded-lg px-3 py-2 max-w-md">
                {remixError}
              </p>
            )}
          </div>
