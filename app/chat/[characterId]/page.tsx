...
import PersonalizeDetailsModal from "@/components/PersonalizeDetailsModal";
import {
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/api/chat/${characterId}`, { signal: controller.signal })
      ...
      .then((data) => {
        if (!data) return;
        setCharacter({ ...data.character, isOwner: Boolean(data.isOwner) });
        ...
        <main className="flex-1 flex flex-col min-h-0 relative">
                ...
                <div className="relative chat-menu">
                  ...
                  {chatMenuOpen && (
                    <div className="absolute right-0 top-full mt-1 w-44 bg-plum-deep border border-parchment/15 rounded-xl shadow-xl py-1 z-50 animate-fade-in">
                      ...
                      <div className="border-t border-parchment/10 my-0.5" />
                      <button onClick={() => { setAvatarModalOpen(true); setChatMenuOpen(false); }} className="w-full text-left px-3 py-2 text-xs text-parchment/80 hover:bg-white/5 transition-colors">🖼 Change portrait</button>
                      <button onClick={() => { setBackgroundModalOpen(true); setChatMenuOpen(false); }} className="w-full text-left px-3 py-2 text-xs text-parchment/80 hover:bg-white/5 transition-colors">🏞 Change backdrop</button>
                      <button onClick={() => { setDetailsModalOpen(true); setChatMenuOpen(false); }} className="w-full text-left px-3 py-2 text-xs text-parchment/80 hover:bg-white/5 transition-colors">🎨 Personalize details</button>
                      <p className="px-3 pb-1.5 text-[10px] text-parchment/35 leading-snug">Only visible to you — the original stays the same for everyone else.</p>
                      {character.isOwner && (
                        <>
                          <div className="border-t border-parchment/10 my-0.5" />
                          <Link href={`/characters/${character.id}/edit`} onClick={() => setChatMenuOpen(false)} className="block w-full text-left px-3 py-2 text-xs text-parchment/80 hover:bg-white/5 transition-colors">✏️ Edit the real character</Link>
                        </>
                      )}
                      <div className="border-t border-parchment/10 my-0.5" />
                      <div className="px-3 py-2">
          ...
          {character && (
            <BackgroundGenerateModal
              ...
              characterName={character.name}
              isExplicit={character.isExplicit}
              currentBackgroundUrl={character.backgroundUrl}
              onClose={() => setBackgroundModalOpen(false)}
              onUpdated={(backgroundUrl) => setCharacter((c) => (c ? { ...c, backgroundUrl } : c))}
            />
          )}

          {character && (
            <PersonalizeDetailsModal
              ...
              onUpdated={(overrides) => setCharacter((c) => (c ? { ...c, ...overrides } : c))}
            />
          )}
