...
export default function Home() {
  ...
  const [preview, setPreview] = useState<PublicCharacter | null>(null);
  ...
  useEffect(() => {
    ...
    let ignore = false;
    apiFetch("/api/characters/public?take=12")
  ...
  const openPreview = (char: PublicCharacter) => setPreview(char);
    ...
    <main className="min-h-screen flex flex-col bg-void relative overflow-hidden aurora-bg">
      ...
      <section className="relative z-10 px-6 md:px-12 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-xs text-parchment/40 mt-2">Real characters from the community. Click to preview.</p>
        </div>
        {characters === null ? (
          ...
          <p className="text-center text-parchment/40 text-sm">No public characters yet. Be the first to share one.</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 max-w-5xl mx-auto">
            {characters.map((char, i) => (
              <button
                ...
                className="group cursor-pointer animate-fade-in-up block text-left"
                style={{ animationDelay: `${0.4 + i * 0.06}s` }}
              >
                <div className="relative aspect-[3/4] rounded-2xl overflow-hidden border border-white/5 card-hover" style={{ background: `linear-gradient(160deg, ${char.accentColor}30, #121218)` }}>
                  {char.avatarUrl ? (
                    <img src={resolveMediaUrl(char.avatarUrl)} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  ) : (
                  ...
                  <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ boxShadow: `inset 0 0 30px ${char.accentColor}20` }} />
                </div>
              </button>
            ))}
          </div>
      ...
      {preview && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center px-6 z-50 animate-fade-in" onClick={closePreview}>
          <div className="w-full max-w-lg rounded-2xl bg-surface-card border border-white/10 shadow-2xl animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <div className="relative aspect-[3/4] sm:aspect-video rounded-t-2xl overflow-hidden bg-surface-raised">
              {preview.avatarUrl ? (
                <img src={resolveMediaUrl(preview.avatarUrl)} alt={preview.name} className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-7xl" style={{ background: `linear-gradient(160deg, ${preview.accentColor}44, #121218)` }}>
                  {preview.avatarEmoji}
                </div>
              )}
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <p className="font-display text-xl text-parchment">{preview.name}</p>
                <p className="text-xs text-parchment/60 mt-1">{preview.tagline}</p>
              </div>
            </div>
            <div className="p-5">
              <p className="text-sm text-parchment/70 leading-relaxed mb-4">{preview.personality}</p>
              <p className="text-xs text-parchment/40 mb-5 italic">&ldquo;{preview.greeting}&rdquo;</p>
              <div className="flex flex-col sm:flex-row gap-2">
                <Link href={`/signup?character=${preview.id}`} className="flex-1 text-center bg-gold text-ink py-2.5 rounded-full font-medium hover:brightness-110 focus-ring btn-shine">
                  Try this character
                </Link>
                <button type="button" onClick={closePreview} className="flex-1 text-center border border-white/15 py-2.5 rounded-full hover:bg-white/5 focus-ring transition-colors">
                  Close
                </button>
              </div>
            </div>
